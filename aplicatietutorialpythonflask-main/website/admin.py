...
from .models import User, Test, Intrebari, Raspunsuri, Penalisation
import pandas as pd
from . import db
...
def admin_add_teams():
    ...
    if request.method == "POST":
        ...
        if username == '' or password == '':
            flash('Ambele campuri trebuie sa fie completate pentru a putea adauga o echipa', category='error')
        ...
        new_team = User(username=username, password=password)
...
@admin.route('/admin_results', methods=['GET', 'POST'])
@login_required
def admin_results():
    if current_user.level != 'admin':
        return redirect(url_for('admin.admin_error'))
    ...
    if 'questions' in session and 'points' in session:
        ...
        return redirect(url_for('views.home'))
    return render_template('admin_results.html', user = current_user, questions = questions, points = points)
...
def admin_delete_table():
    ...
    tables = {"test": Test, "user" : User, "intrebari": Intrebari, "raspunsuri": Raspunsuri}
...
def admin_add_points():
    ...
    if request.method == 'POST':
        ...
        new_team = Penalisation(number = number, username=user.username, id=id_echipa, penalizare=puncte, motiv=motiv)
        if "quiz" not in motiv:
...
@admin.route('/admin_add_penalisations', methods=['GET', 'POST'])
@login_required
def admin_add_penalisations():
    if current_user.level != 'admin':
        return redirect(url_for('admin.admin_error'))
    echipe = User.query.filter_by(level='team').all()
    ...
    if request.method == 'POST':
            number= request.form.get("number")
            id_echipa = request.form.get("id_echipa")
            penalizare = request.form.get("penalizare")
            motiv = request.form.get("motiv")
            user = User.query.filter_by(id=id_echipa).first()
            user.change_points(user.punctaj - int(penalizare))
            new_team = Penalisation( number= number,username = user.username, id = id_echipa, penalizare = penalizare, motiv = motiv)
            db.session.add(new_team)
            db.session.commit()
            db.session.close_all()
            return redirect(url_for("admin.admin_add_penalisations"))

    return render_template('admin_add_penalisations.html', user=current_user, echipe=echipe)
